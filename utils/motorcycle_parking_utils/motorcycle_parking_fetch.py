...
get all spcified data for motorcycle parking from Motorcycle parking
'''

import re
from ..raw_data_fetch import fetch_raw_data

URL = "https://opendata.vancouver.ca/api/explore/v2.1/catalog/datasets/motorcycle-parking/exports/json?lang=en&timezone=America%2FLos_Angeles"


# fetch data

def get_all_parking_type() -> str:
    raw_data = fetch_raw_data(URL)
    parking_type_regex = r'.(Metered\s*Motorcycle\s*Parking|Non-Metered\s*Motorcycle Parking).'  # Regex to find meter IDs
    parking_type = re.findall(parking_type_regex, raw_data)  # Find all matches
    return parking_type
...
def get_all_rate_mf_9a_6p():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_mf_9a_6p":\s*\W(null|[^"]*)'
...
def get_all_rate_mf_6p_10():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_mf_6p_10":\s*\W(null|[^"]*)'
    rates = re.findall(rate_regex, raw_data)  # Find all matches
    return rates
...
def get_all_rate_sa_9a_6p():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_sa_9a_6p":\s*\W(null|[^"]*)'
    rates = re.findall(rate_regex, raw_data)  # Find all matches
    return rates
...
def get_all_rate_sa_6p_10():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_sa_6p_10":\s*\W(null|[^"]*)'
    rates = re.findall(rate_regex, raw_data)  # Find all matches
    return rates
...
def get_all_rate_su_9a_6p():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_su_9a_6p":\s*\W(null|[^"]*)'
    rates = re.findall(rate_regex, raw_data)  # Find all matches
    return rates
...
def get_all_rate_su_6p_10():
    raw_data = fetch_raw_data(URL)
    rate_regex = r'"r_su_6p_10":\s*\W(null|[^"]*)'
    rates = re.findall(rate_regex, raw_data)  # Find all matches
    return rates
